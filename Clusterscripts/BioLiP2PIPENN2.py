# Including function for nucleotide bindingsites (lines 138-192)
...
# function that will create an extra column that will make a distinction between DNA and RNA bindingsites
# DNA: A1 B2 C3 and/or RNA: D4 E5 F6
# Function that creates an extra column distinguishing DNA and RNA binding sites
# DNA: A1 B2 C3 and/or RNA: D4 E5 F6
def nucleotides(ClusteredFile):
    ...
    with open(ClusteredFile, 'r') as file:
        ...
        for row in file:
            ...
            binding_type = cols[4].strip().lower() # DNA, RNA or both
            sequence = cols[20].strip()

            # Binding residues
            residues = cols[8].strip()

            # Take into account that duplicates are removes; aggregated
            if sequence not in sequence_binding:
            ...
            if binding_type == "dna":
                sequence_binding[sequence]["DNA"].update(residues.split())

            elif binding_type == "rna":
                sequence_binding[sequence]["RNA"].update(residues.split())

    nuc_binding = ["n_binding"]

    for sequence, bindings in sequence_binding.items():
    
        # sort by residue position
        dna_sites = sorted(bindings["DNA"], key=lambda x: int(x[1:]))
        rna_sites = sorted(bindings["RNA"], key=lambda x: int(x[1:]))

        output = []

        # Clean output as descriped above function
        if dna_sites:
            output.append("DNA: " + " ".join(dna_sites))

        if rna_sites:
            output.append("RNA: " + " ".join(rna_sites))

        nuc_binding.append(" | ".join(output))
...
def main():
    ...
    # extra function for nucleotides
    OutData.append(nucleotides(ClusteredFile))
